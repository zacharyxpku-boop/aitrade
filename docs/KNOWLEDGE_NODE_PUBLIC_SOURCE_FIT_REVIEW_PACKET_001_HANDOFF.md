...
- Status: node_public_source_fit_packet_001_handoff_ready_blocked_on_real_input
- Packet: node-public-source-fit-batch-001-packet
- Module: K线与价格行为
- Input copy: docs/reviewer-inputs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_001_INPUT_COPY_TEMPLATE.json
- Review rows: 60
...
|---:|---|---|---|---|
| 1 | inspect_packet_input_copy_template | node_public_source_fit_packet_input_copy_template_ready_blank | `npm.cmd run check:knowledge-node-public-source-fit-review-packet-input-copy-template` | Stop if the file is not packet 001, has fewer than 60 rows, or any row already claims learnerCitationApproved/copy approval. |
| 2 | validate_packet_input_copy | blocked_missing_real_reviewer_source_fit_input | `npm.cmd run validate:knowledge-node-public-source-fit-review-packet-input-copy-template && npm.cmd run check:knowledge-node-public-source-fit-review-packet-input-copy-template` | Stop while readyRows is 0, blockedRows is 60, missingFieldRows is 60, or realHumanInputEntries is 0. |
| 3 | rebuild_merge_preview | packet_merge_preview_blocked_missing_ready_packet_input | `npm.cmd run build:knowledge-node-public-source-fit-review-packet-merge-preview && npm.cmd run check:knowledge-node-public-source-fit-review-packet-merge-preview` | Stop unless mappedRows is 60, missingTargetRows is 0, and mergeAllowedNow is true after real review. |
| 4 | run_merge_apply_dry_run | blocked_no_ready_merge_rows | `npm.cmd run apply:knowledge-node-public-source-fit-review-packet-merge && npm.cmd run check:knowledge-node-public-source-fit-review-packet-merge-apply-report` | Stop if writtenRows is nonzero in dry-run or any row willWrite before manual approval. |
