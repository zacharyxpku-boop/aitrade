...
const validationMdPath = "docs/LOCAL_COURSE_5_WAVE_1_P0_REVIEWER_INPUT_VALIDATION.md";
const inputPath = "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_1_P0_REVIEWER_INPUT_TEMPLATE.json";
...
if (validation.validationStatus !== "course_5_wave_1_p0_reviewer_input_blocked_missing_or_invalid_real_input") fail(`unexpected validationStatus: ${validation.validationStatus}`);
if (validation.validationMode !== "wave_1_p0_real_reviewer_input_gate") fail("unexpected validation mode");
if (validation.inputRows !== 18 || validation.pdfRows !== 6 || validation.zipRows !== 12) fail("Wave 1 validation counts drift");
if (validation.readyRows !== 0 || validation.blockedRows !== 18) fail("blank Wave 1 input should be fully blocked");
if (validation.missingFieldRows !== 18) fail("all blank Wave 1 rows should have missing fields");
if (validation.qualityIssueRows !== 0 || validation.forbiddenHitRows !== 0) fail("blank Wave 1 rows should not have quality or forbidden hits");
if (validation.acceptedForWave1SemanticReviewRows !== 0 || validation.acceptedForModuleDistillationRows !== 0 || validation.acceptedForDeletionReadinessRows !== 0) fail("validation must not accept rows");
if (validation.sourceFolderMayBeDeleted !== false || validation.learnerReadyModules !== 0) fail("release/deletion boundary drift");

if (!Array.isArray(validation.validationRows) || validation.validationRows.length !== 18) fail("validationRows must cover 18 rows");
...
for (const row of validation.validationRows) {
  ...
  if (row.validationStatus !== "blocked_missing_or_invalid_wave_1_reviewer_input") fail(`blank validation row must be blocked: ${row.reviewRowId}`);
  if (row.readyForWave1SemanticReviewGate !== false) fail(`blank validation row cannot be ready: ${row.reviewRowId}`);
  ...
  if (row.nextGate !== "fill_real_wave_1_reviewer_fields_then_revalidate") fail(`unexpected next gate: ${row.reviewRowId}`);
}
