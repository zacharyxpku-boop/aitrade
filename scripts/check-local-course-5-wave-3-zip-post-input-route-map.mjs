...
const routeMapPath = "docs/LOCAL_COURSE_5_WAVE_3_ZIP_POST_INPUT_ROUTE_MAP.json";
const routeMapMdPath = "docs/LOCAL_COURSE_5_WAVE_3_ZIP_POST_INPUT_ROUTE_MAP.md";
const routeMapHtmlPath = "docs/local-course-5-wave-3-zip-post-input-route-map.html";
...
if (routeMap.routeMapStatus !== "course_5_wave_3_zip_post_input_route_map_ready_all_rows_blocked") fail(`unexpected routeMapStatus: ${routeMap.routeMapStatus}`);
if (routeMap.routeMode !== "post_input_route_map_for_wave_3_zip_reviewer_rows") fail("unexpected route mode");
if (routeMap.waveId !== "wave_3_locally_resolvable_zip_visuals") fail("unexpected wave id");
if (routeMap.routeRows !== 61 || routeMap.pdfRouteRows !== 0 || routeMap.zipRouteRows !== 61) fail("route counts drift");
if (routeMap.readyRows !== 0 || routeMap.blockedRows !== 61 || routeMap.semanticMergePreviewReadyRows !== 0) fail("blank Wave 3 ZIP route rows should be blocked");
...
if (routeMap.acceptedForWave3SemanticReviewRows !== 0 || routeMap.acceptedForModuleDistillationRows !== 0 || routeMap.acceptedForDeletionReadinessRows !== 0) fail("route map must not accept rows");
...
if (!Array.isArray(routeMap.routeRowsDetail) || routeMap.routeRowsDetail.length !== 61) fail("routeRowsDetail must cover 61 rows");
...
for (const row of routeMap.routeRowsDetail) {
  ...
  if (row.sourceType !== "zip") fail(`route row must be ZIP: ${row.reviewRowId}`);
  if (row.readyForWave3SemanticReviewGate !== false) fail(`route row should not be ready: ${row.reviewRowId}`);
  if (row.routeStatus !== "blocked_before_wave_3_zip_semantic_merge_preview") fail(`route row should be blocked: ${row.reviewRowId}`);
  ...
  if (row.downstreamRouteMap !== routeMap.sourceZipRouteMap) fail(`ZIP downstream route drift: ${row.reviewRowId}`);
...
for (const phrase of [
  ...
  "validation-ready wave 3 zip reviewer rows",
  "zip semantic merge preview",
  "public grounding",
