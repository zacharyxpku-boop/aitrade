...
const inputPath = argValue("--input", "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_1_P0_REVIEWER_INPUT_TEMPLATE.json");
const packPath = argValue("--pack", "docs/LOCAL_COURSE_5_WAVE_1_P0_EXECUTION_PACK.json");
const outputJsonPath = argValue("--output-json", "docs/LOCAL_COURSE_5_WAVE_1_P0_REVIEWER_INPUT_VALIDATION.json");
const outputMdPath = argValue("--output-md", "docs/LOCAL_COURSE_5_WAVE_1_P0_REVIEWER_INPUT_VALIDATION.md");
...
assertBoundary("Wave 1 reviewer input", input);
assertBoundary("Wave 1 execution pack", pack);

if (!Array.isArray(input.rows) || input.rows.length !== 18) fail("expected 18 Wave 1 reviewer input rows");
if (!Array.isArray(pack.sampleRowsDetail) || pack.sampleRowsDetail.length !== 18) fail("expected 18 Wave 1 execution samples");
...
const validationRows = input.rows.map((row) => {
  assertBoundary(`Wave 1 reviewer input ${row.reviewRowId}`, row);
  ...
  return {
    ...
    validationStatus: rowIssues.length ? "blocked_missing_or_invalid_wave_1_reviewer_input" : "ready_for_wave_1_semantic_review_gate",
    readyForWave1SemanticReviewGate: rowIssues.length === 0,
    missingFields,
    ...
    nextGate: rowIssues.length
      ...
      : "route_ready_wave_1_row_to_source_type_priority_route_map_after_public_grounding_check",
  };
});

const readyRows = validationRows.filter((row) => row.readyForWave1SemanticReviewGate).length;
...
const report = {
  ...
  validationStatus: readyRows === validationRows.length
    ? "course_5_wave_1_p0_reviewer_input_valid_release_still_locked"
    : "course_5_wave_1_p0_reviewer_input_blocked_missing_or_invalid_real_input",
  validationMode: "wave_1_p0_real_reviewer_input_gate",
  inputPath,
  ...
  forbiddenPhrases,
  validationRows,
  completionRule: "Wave 1 P0 reviewer input validation passes only when all 18 rows contain real reviewer-owned ZIP visual or PDF OCR/visual fields, no copied machine summaries, no forbidden trading advice language, public grounding remains required, and no release/delete boundary drifts.",
  boundary: "Course 5 Wave 1 P0 reviewer input validation is private reviewer-facing education operations material. It validates reviewer-owned input for the first three P0 source blockers and their representative ZIP/PDF samples. It does not generate reviewer conclusions, accept machine drafts as human review, delete files, approve learner-facing release, approve source-folder deletion, copy private source wording, provide stock recommendations, live signals, return promises, broker workflows, automation, real-money guidance, write authorization, or production readiness.",
};

fs.writeFileSync(outputJsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
fs.writeFileSync(outputMdPath, [
  "# Course 5 Wave 1 P0 Reviewer Input Validation",
  "",
  ...
  "",
  ...validationRows
    .filter((row) => !row.readyForWave1SemanticReviewGate)
    .slice(0, 18)
