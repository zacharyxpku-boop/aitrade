...
const validationPath = "docs/LOCAL_COURSE_5_PDF_OCR_VISUAL_REVIEW_INPUT_VALIDATION.json";
const validationMdPath = "docs/LOCAL_COURSE_5_PDF_OCR_VISUAL_REVIEW_INPUT_VALIDATION.md";
...
if (validation.validationStatus !== "course_5_pdf_ocr_visual_review_input_blocked_missing_or_invalid_real_ocr_or_visual_input") fail(`unexpected validationStatus: ${validation.validationStatus}`);
if (validation.validationMode !== "pdf_ocr_visual_real_reviewer_input_gate") fail("unexpected validation mode");
if (validation.inputRows !== 121 || validation.readyRows !== 0 || validation.blockedRows !== 121) fail("PDF validation should start fully blocked");
if (validation.missingFieldRows !== 121) fail("all blank PDF input rows should have missing fields");
...
if (!Array.isArray(validation.validationRows) || validation.validationRows.length !== 121) fail("validationRows must cover 121 rows");
...
for (const row of validation.validationRows) {
  ...
  if (row.validationStatus !== "blocked_missing_or_invalid_real_ocr_or_visual_input") fail(`blank validation row must be blocked: ${row.reviewRowId}`);
  if (row.readyForPdfVisualSemanticReviewGate !== false) fail(`blank validation row cannot be ready: ${row.reviewRowId}`);
  ...
  if (row.nextGate !== "fill_real_ocr_or_visual_reviewer_fields_then_revalidate") fail(`unexpected next gate: ${row.reviewRowId}`);
}

const boundaryText = `${validation.boundary || ""} ${validation.completionRule || ""}`.toLowerCase();
for (const phrase of [
  "private reviewer-facing education operations",
  "human/ocr-owned page observations",
  "manual/ocr text notes",
