...
const auditPath = "docs/LOCAL_COURSE_5_WAVE_4_PDF_AI_VISUAL_REVIEW_COVERAGE_AUDIT.json";
const auditMdPath = "docs/LOCAL_COURSE_5_WAVE_4_PDF_AI_VISUAL_REVIEW_COVERAGE_AUDIT.md";
...
if (audit.auditStatus !== "course_5_wave_4_pdf_ai_visual_review_all_samples_covered_release_and_deletion_blocked") fail("unexpected audit status");
if (audit.batchCount !== 3) fail(`expected 3 batches, got ${audit.batchCount}`);
if (audit.templateRows !== 27) fail(`expected 27 template rows, got ${audit.templateRows}`);
if (audit.coveredRows !== 27) fail(`expected 27 covered rows, got ${audit.coveredRows}`);
...
if (audit.readyForHumanConfirmationRows !== 27) fail(`expected 27 ready rows, got ${audit.readyForHumanConfirmationRows}`);
...
if (!Array.isArray(audit.coverageRows) || audit.coverageRows.length !== 27) fail("coverage rows must contain 27 rows");
