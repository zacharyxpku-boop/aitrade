...
const validationPath = "docs/LOCAL_COURSE_5_PDF_OCR_PRIORITY_SLICE_INPUT_VALIDATION.json";
...
const routeRows = validation.validationRows.map((row) => {
  ...
  const ready = row.readyForPrioritySliceSemanticReviewGate === true;
  return {
    ...
    readyForPrioritySliceSemanticReviewGate: ready,
    routeStatus: ready
      ? "ready_for_semantic_merge_preview_public_grounding_required"
      : "blocked_before_semantic_merge_preview",
    semanticMergePreviewAllowedNow: ready,
    moduleDistillationAllowedNow: false,
    deletionEvidenceAllowedNow: false,
    nextGate: ready
      ? "build_pdf_priority_slice_semantic_merge_preview_then_public_grounding_review"
      : row.nextGate,
...
const readyRows = routeRows.filter((row) => row.readyForPrioritySliceSemanticReviewGate).length;
...
const routeMap = {
  ...
  routeMapStatus: readyRows > 0
    ? "course_5_pdf_ocr_priority_slice_route_map_has_ready_rows_release_and_deletion_blocked"
    : "course_5_pdf_ocr_priority_slice_route_map_ready_all_rows_blocked",
  routeMode: "post_input_route_map_for_first_controlled_pdf_ocr_slice",
  ...
  nextOperationalGates: [
    "Fill real OCR or manual visual reviewer fields for the 36 priority slice rows.",
    "Re-run priority slice input validation until rows become ready.",
    "Build a semantic merge preview only for validation-ready rows.",
  ...
  commands: [
    "npm.cmd run validate:local-course-5-pdf-ocr-priority-slice-input",
    "npm.cmd run check:local-course-5-pdf-ocr-priority-slice-input-validation",
    "npm.cmd run build:local-course-5-pdf-ocr-priority-slice-route-map",
    "npm.cmd run check:local-course-5-pdf-ocr-priority-slice-route-map",
    "npm.cmd run verify",
  ],
  completionRule: "This route map is complete when all 36 priority-slice validation rows are assigned a post-input route, ready rows are isolated for future semantic merge preview, and module/deletion gates remain closed until real accepted reviewer evidence and public grounding checks pass.",
  boundary,
};
...
fs.writeFileSync(outputMd, [
  "# Course 5 PDF OCR Priority Slice Route Map",
  "",
  ...
  "## First Rows",
  "",
  "| Review row | Priority | Status | Next gate |",
  "|---|---|---|---|",
  ...routeRows.slice(0, 20).map((row) => `| ${row.reviewRowId} | ${row.priorityBand} | ${row.routeStatus} | ${row.nextGate} |`),
  "",
  "## Boundary",
  "",
  boundary,
...
<head>
  ...
  <title>Course 5 PDF OCR Priority Slice Route Map</title>
  <style>
...
<body>
  <header>
    <h1>Course 5 PDF OCR Priority Slice Route Map</h1>
  ...
  <main>
    <table>
      <thead><tr><th>Review Row</th><th>Priority</th><th>Route Status</th><th>Next Gate</th></tr></thead>
      <tbody>${rowHtml}</tbody>
    </table>
  </main>
