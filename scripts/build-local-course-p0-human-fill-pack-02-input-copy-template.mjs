...
const outputJsonPath = "docs/LOCAL_COURSE_P0_HUMAN_FILL_PACK_02_INPUT_COPY_TEMPLATE.json";
const outputMdPath = "docs/LOCAL_COURSE_P0_HUMAN_FILL_PACK_02_INPUT_COPY_TEMPLATE.md";
...
const inputEntries = (pack.packCards || []).map((card) => ({
  ...
  acceptanceCriteria: [
    ...
    "Risk rewrite notes address every riskTermFlag, especially signal, forecast, support/resistance, homework, and source-recommendation language.",
    "Public reference notes identify candlestick terminology, pattern taxonomy, support/resistance claims, or source recommendations that need public grounding.",
    "Originality notes confirm private course wording is not copied into learner-facing lessons.",
  ],
  nextGate: "validate_pack_02_input_then_apply_dry_run_only",
}));
...
const inputCopyTemplate = {
  ...
  approvalStatus: "not_approved",
  fixtureOnly: false,
  templateStatus: "pack_02_input_copy_blank",
  ...
  inputEntries,
  usage: [
    "Copy this file before a human reviewer fills pack 02.",
    ...
    "Run npm.cmd run check:local-course-p0-human-fill-pack-02-input-copy-template before and after filling a copy.",
    "Run validator and apply dry-run against the copied file; do not use --write until explicitly authorized after review.",
  ],
  completionRule: "This input copy template is only a blank carrier for pack 02. It is not ready until a human reviewer fills all required fields, every risk flag is addressed, validation succeeds, and apply dry-run reports ready entries with writtenEntries:0.",
  boundary: "Pack 02 input copy template is blank reviewer input material. It does not perform OCR, fill reviewer fields, approve learner-facing release, copy private course wording into lessons, provide trading advice, stock recommendations, live signals, return promises, broker workflows, automation, or real-money guidance.",
};

fs.writeFileSync(outputJsonPath, `${JSON.stringify(inputCopyTemplate, null, 2)}\n`, "utf8");
fs.writeFileSync(outputMdPath, [
  "# Local Course P0 Human Fill Pack 02 Input Copy Template",
  "",
  "Blank reviewer-input copy template scoped to fill pack 02.",
  "",
