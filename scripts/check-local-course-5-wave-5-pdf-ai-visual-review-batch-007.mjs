...
const inputPath = "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_5_PDF_AI_VISUAL_REVIEW_BATCH_007_INPUT.json";
const validationPath = "docs/LOCAL_COURSE_5_WAVE_5_PDF_AI_VISUAL_REVIEW_BATCH_007_VALIDATION.json";
...
if (input.inputTemplateStatus !== "course_5_wave_5_pdf_ai_visual_review_batch_007_final_thirteen_rows_ready_for_reviewer_confirmation") fail("unexpected input status");
if (validation.validationStatus !== "course_5_wave_5_pdf_ai_visual_review_batch_007_final_rows_ready_release_and_deletion_blocked") fail("unexpected validation status");
if (validation.validationMode !== "wave_5_pdf_ai_visual_review_batch_007_gate") fail("unexpected validation mode");
...
if (validation.blockedRows !== 72) fail(`expected 72 blocked rows, got ${validation.blockedRows}`);
if (validation.missingFieldRows !== 72) fail(`expected 72 missing field rows, got ${validation.missingFieldRows}`);
...
const expectedIds = input.rows.slice(72, 85).map((row) => row.reviewRowId);
