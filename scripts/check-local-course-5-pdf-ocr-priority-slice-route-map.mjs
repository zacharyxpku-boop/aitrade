...
const routeMapPath = "docs/LOCAL_COURSE_5_PDF_OCR_PRIORITY_SLICE_ROUTE_MAP.json";
const routeMapMdPath = "docs/LOCAL_COURSE_5_PDF_OCR_PRIORITY_SLICE_ROUTE_MAP.md";
const routeMapHtmlPath = "docs/local-course-5-pdf-ocr-priority-slice-route-map.html";
...
if (routeMap.routeMapStatus !== "course_5_pdf_ocr_priority_slice_route_map_ready_all_rows_blocked") fail(`unexpected routeMapStatus: ${routeMap.routeMapStatus}`);
if (routeMap.routeMode !== "post_input_route_map_for_first_controlled_pdf_ocr_slice") fail("unexpected route mode");
if (routeMap.routeRows !== 36 || routeMap.readyRows !== 0 || routeMap.blockedRows !== 36) fail("route row counts drift");
...
if (!Array.isArray(routeMap.routeRowsDetail) || routeMap.routeRowsDetail.length !== 36) fail("routeRowsDetail must cover 36 rows");
...
for (const row of routeMap.routeRowsDetail) {
  ...
  if (row.readyForPrioritySliceSemanticReviewGate !== false) fail(`route row should not be ready: ${row.reviewRowId}`);
  if (row.routeStatus !== "blocked_before_semantic_merge_preview") fail(`route row should be blocked: ${row.reviewRowId}`);
