...
const routeMapPath = "docs/LOCAL_COURSE_5_WAVE_1_P0_POST_INPUT_ROUTE_MAP.json";
const routeMapMdPath = "docs/LOCAL_COURSE_5_WAVE_1_P0_POST_INPUT_ROUTE_MAP.md";
const routeMapHtmlPath = "docs/local-course-5-wave-1-p0-post-input-route-map.html";
...
if (routeMap.routeMapStatus !== "course_5_wave_1_p0_post_input_route_map_ready_all_rows_blocked") fail(`unexpected routeMapStatus: ${routeMap.routeMapStatus}`);
if (routeMap.routeMode !== "post_input_route_map_for_wave_1_p0_reviewer_rows") fail("unexpected route mode");
if (routeMap.waveId !== "wave_1_p0_space_and_curriculum_blockers") fail("unexpected wave id");
if (routeMap.routeRows !== 18 || routeMap.pdfRouteRows !== 6 || routeMap.zipRouteRows !== 12) fail("route counts drift");
if (routeMap.readyRows !== 0 || routeMap.blockedRows !== 18 || routeMap.semanticMergePreviewReadyRows !== 0) fail("blank Wave 1 route rows should be blocked");
...
if (routeMap.acceptedForWave1SemanticReviewRows !== 0 || routeMap.acceptedForModuleDistillationRows !== 0 || routeMap.acceptedForDeletionReadinessRows !== 0) fail("route map must not accept rows");
...
if (!Array.isArray(routeMap.routeRowsDetail) || routeMap.routeRowsDetail.length !== 18) fail("routeRowsDetail must cover 18 rows");
...
for (const row of routeMap.routeRowsDetail) {
  ...
  if (row.readyForWave1SemanticReviewGate !== false) fail(`route row should not be ready: ${row.reviewRowId}`);
  if (row.routeStatus !== "blocked_before_wave_1_semantic_merge_preview") fail(`route row should be blocked: ${row.reviewRowId}`);
...
for (const phrase of [
  ...
  "validation-ready wave 1 reviewer rows",
