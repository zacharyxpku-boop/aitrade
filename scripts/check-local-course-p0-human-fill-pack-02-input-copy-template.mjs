...
const validationPath = "docs/LOCAL_COURSE_P0_HUMAN_FILL_PACK_02_INPUT_COPY_VALIDATION.json";
...
if (template.templateStatus !== "pack_02_input_copy_blank") fail(`unexpected templateStatus: ${template.templateStatus}`);
if (template.packId !== "local_course_p0_human_fill_pack_02") fail(`unexpected packId: ${template.packId}`);
if (template.totalEntries !== 4 || entries.length !== 4) fail(`expected 4 entries, got ${template.totalEntries}/${entries.length}`);
if (template.manualTranscriptionEntries !== 4 || template.sourceReplacementEntries !== 0) fail("pack 02 copy must be manual-only");
...
for (const entry of entries) {
  ...
  if (!Array.isArray(entry.packQualityRequirements?.riskTermFlags) || entry.packQualityRequirements.riskTermFlags.length < 5) {
  ...
  if (entry.nextGate !== "validate_pack_02_input_then_apply_dry_run_only") fail(`${entry.id} next gate drift`);
}
