...
const inputPath = "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_005_INPUT.json";
const validationPath = "docs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_005_VALIDATION.json";
...
if (input.inputTemplateStatus !== "course_5_wave_3_zip_ai_visual_review_batch_005_final_rows_ready_for_reviewer_confirmation") fail("unexpected input status");
if (validation.validationStatus !== "course_5_wave_3_zip_ai_visual_review_batch_005_final_rows_ready_release_and_deletion_blocked") fail("unexpected validation status");
if (validation.validationMode !== "wave_3_zip_ai_visual_review_batch_005_gate") fail("unexpected validation mode");
if (validation.inputRows !== 61) fail(`expected 61 input rows, got ${validation.inputRows}`);
if (validation.batchRows !== 13) fail(`expected 13 batch rows, got ${validation.batchRows}`);
if (validation.readyRows !== 13) fail(`expected 13 ready rows, got ${validation.readyRows}`);
if (validation.blockedRows !== 48) fail(`expected 48 blocked rows, got ${validation.blockedRows}`);
if (validation.missingFieldRows !== 48) fail(`expected 48 missing field rows, got ${validation.missingFieldRows}`);
