...
const batchPath = "docs/LOCAL_COURSE_HIGH_RES_TRANSCRIPTION_CANDIDATE_BATCH_01.json";
...
if (batch.documentId !== "corpus_1313") fail(`unexpected documentId: ${batch.documentId}`);
...
for (const page of pages) {
  ...
  if (!Array.isArray(page.riskTermFlags) || page.riskTermFlags.length < 2) fail(`${page.id} needs risk-term flags`);
...
if (!riskCounts.entry_language || !riskCounts.position_language || !riskCounts.specific_price_levels) {
  fail("risk term counts should preserve operational-language warning flags");
}
