...
const outputJsonPath = "docs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_COVERAGE_AUDIT.json";
const outputMdPath = "docs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_COVERAGE_AUDIT.md";
const reviewedAt = "2026-06-25T00:00:00.000+08:00";

const batchSpecs = [
  { id: "batch_001", path: "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_001_INPUT.json", validationPath: "docs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_001_VALIDATION.json" },
  { id: "batch_002", path: "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_002_INPUT.json", validationPath: "docs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_002_VALIDATION.json" },
  { id: "batch_003", path: "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_003_INPUT.json", validationPath: "docs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_003_VALIDATION.json" },
  { id: "batch_004", path: "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_004_INPUT.json", validationPath: "docs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_004_VALIDATION.json" },
  { id: "batch_005", path: "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_005_INPUT.json", validationPath: "docs/LOCAL_COURSE_5_WAVE_3_ZIP_AI_VISUAL_REVIEW_BATCH_005_VALIDATION.json" },
];
...
for (const batch of batchSpecs) {
  ...
  if (validation.sourceFolderMayBeDeleted !== false) fail(`${batch.id} must not open source folder deletion`);
...
const coverageRows = expectedIds.map((id) => {
  ...
  return {
    ...
    archiveImageName: templateRow.archiveImageName,
...
if (template.rows.length !== 61) fail(`expected 61 template rows, got ${template.rows.length}`);
if (coveredRows.length !== 61) fail(`expected 61 covered rows, got ${coveredRows.length}`);
...
const artifact = {
  ...
  auditStatus: "course_5_wave_3_zip_ai_visual_review_all_samples_covered_release_and_deletion_blocked",
  ...
  nextOperationalGates: [
    "Use this audit as the Wave 3 ZIP visual review closure proof.",
    "Keep public grounding and originality review required before module merge.",
    "Continue with PDF/OCR or reviewer-confirmation blockers before recomputing deletion readiness.",
  ],
  completionRule: "Wave 3 ZIP AI visual review coverage is complete when every row in the Wave 3 ZIP reviewer input template is covered exactly once by Batch 001 through Batch 005 while all release, module merge, and source-folder deletion gates remain closed.",
  boundary: "Private education-operations audit for Course 5 Wave 3 ZIP chart sample coverage. It summarizes model-assisted visual review coverage and keeps learner release, module merge, write authorization, and source-folder removal blocked pending reviewer confirmation, public grounding, originality review, and explicit approval.",
};
...
fs.writeFileSync(outputMdPath, `# Course 5 Wave 3 ZIP AI Visual Review Coverage Audit
