...
const outputJson = "docs/LOCAL_COURSE_5_WAVE_1_P0_EXECUTION_PACK.json";
const outputMd = "docs/LOCAL_COURSE_5_WAVE_1_P0_EXECUTION_PACK.md";
const outputHtml = "docs/local-course-5-wave-1-p0-execution-pack.html";

const boundary = "Course 5 Wave 1 P0 execution pack is private reviewer-facing education operations material. It isolates the first three P0 source blockers and their representative ZIP/PDF visual samples for real reviewer-owned visual or OCR input, future validation, semantic merge preview, public grounding, and deletion-readiness recomputation. It does not perform OCR, generate reviewer conclusions, accept machine drafts as human review, merge content into learner-facing modules, delete files, approve source-folder deletion, approve learner-facing release, copy private source wording, provide stock recommendations, live signals, return promises, broker workflows, automation, real-money guidance, write authorization, or production readiness.";
...
for (const [name, artifact] of Object.entries({ priorityPlan, pdfSlice, zipSlice, deletion })) {
  assertBoundary(name, artifact);
}

const waveRows = priorityPlan.priorityRows.filter((row) => row.resolutionWave === "wave_1_p0_space_and_curriculum_blockers");
...
const pdfSamples = pdfSlice.sampleRows.filter((row) => waveRecordIds.has(row.recordId));

const sourceRows = waveRows.map((row) => {
  const samples = row.extension === ".zip"
    ? zipSamples.filter((sample) => sample.recordId === row.recordId)
    : pdfSamples.filter((sample) => sample.recordId === row.recordId);
  return {
    ...
    requiredReviewerMode: row.extension === ".zip"
      ? "zip_visual_reviewer_owned_observation"
      : "pdf_ocr_or_visual_reviewer_owned_observation",
    nextValidationArtifact: row.extension === ".zip"
      ? "docs/LOCAL_COURSE_5_ZIP_VISUAL_PRIORITY_SLICE_INPUT_VALIDATION.json"
      : "docs/LOCAL_COURSE_5_PDF_OCR_PRIORITY_SLICE_INPUT_VALIDATION.json",
    nextRouteMapArtifact: row.extension === ".zip"
      ? "docs/LOCAL_COURSE_5_ZIP_VISUAL_PRIORITY_SLICE_ROUTE_MAP.json"
      : "docs/LOCAL_COURSE_5_PDF_OCR_PRIORITY_SLICE_ROUTE_MAP.json",
...
const sampleRows = [
  ...zipSamples.map((sample) => ({
    ...
    reviewerPrompt: "Describe visible chart structures, labels, and representativeness in reviewer-owned words; do not copy source wording or infer a trading signal.",
    requiredFields: [
  ...
  const sourceOrderA = waveRows.find((row) => row.recordId === a.recordId)?.resolutionOrder || 99;
  const sourceOrderB = waveRows.find((row) => row.recordId === b.recordId)?.resolutionOrder || 99;
...
const executionPack = {
  ...
  approvalStatus: "not_approved",
  writeAllowedNow: false,
  executionPackStatus: "course_5_wave_1_p0_execution_pack_ready_blocked_on_real_reviewer_input",
  waveId: "wave_1_p0_space_and_curriculum_blockers",
  ...
  pdfSourceRows: sourceRows.filter((row) => row.extension === ".pdf").length,
  zipSourceRows: sourceRows.filter((row) => row.extension === ".zip").length,
  sampleRows: sampleRows.length,
  pdfSampleRows: sampleRows.filter((row) => row.sourceType === "pdf").length,
  zipSampleRows: sampleRows.filter((row) => row.sourceType === "zip").length,
  ...
  ocrBlockedSourceRows: sourceRows.filter((row) => row.extension === ".pdf").length,
  ...
  nextOperationalGates: [
    "Fill reviewer-owned input for the 12 ZIP image samples and re-run ZIP priority validation.",
    "Provide OCR or reviewer-owned visual/text notes for the 6 PDF representative pages and re-run PDF priority validation.",
    "Only route validation-ready rows into semantic merge preview.",
    "Run public grounding and originality review before any module distillation.",
    "Keep Course 5 source-folder deletion blocked until accepted replacement evidence exists for every follow-up row.",
  ],
  commands: [
    "npm.cmd run build:local-course-5-wave-1-p0-execution-pack",
    "npm.cmd run check:local-course-5-wave-1-p0-execution-pack",
    "npm.cmd run verify",
  ],
  completionRule: "This Wave 1 pack is complete when the 3 P0 blocker source rows and their 18 representative ZIP/PDF sample rows are isolated with reviewer-owned input requirements, validation routes, and release/deletion gates closed.",
  boundary,
};
...
fs.writeFileSync(outputMd, [
  ...
  `- PDF/ZIP sample rows: ${executionPack.pdfSampleRows}/${executionPack.zipSampleRows}`,
  `- Affected modules: ${executionPack.affectedModules.join(", ")}`,
  `- Source folder may be deleted: ${executionPack.sourceFolderMayBeDeleted}`,
  "",
  "## Source Rows",
  "",
  "| Order | Type | Samples | Modules | Next validation |",
  "|---:|---|---:|---|---|",
  ...sourceRows.map((row) => `| ${row.resolutionOrder} | ${row.extension} | ${row.sampleRows} | ${row.moduleIds.join(", ")} | ${row.nextValidationArtifact} |`),
  "",
  "## Boundary",
  "",
  boundary,
...
const rowHtml = sourceRows.map((row) => `
        <tr>
          ...
          <td>${htmlEscape(row.nextValidationArtifact)}</td>
        </tr>`).join("");
...
<head>
  ...
  <title>Course 5 Wave 1 P0 Execution Pack</title>
  <style>
...
<body>
  <header>
    <h1>Course 5 Wave 1 P0 Execution Pack</h1>
  ...
  <main>
    <table>
      <thead><tr><th>Order</th><th>Type</th><th>Samples</th><th>Modules</th><th>Next Validation</th></tr></thead>
      <tbody>${rowHtml}</tbody>
    </table>
  </main>
...
console.log(JSON.stringify({
  ...
  pdfSampleRows: executionPack.pdfSampleRows,
