...
const batchPath = "docs/LOCAL_COURSE_HIGH_RES_TRANSCRIPTION_CANDIDATE_BATCH_05.json";
...
if (batch.candidatePages !== 3 || pages.length !== 3) fail(`expected 3 candidate pages, got ${batch.candidatePages}/${pages.length}`);
if (batch.acceptedForP0OverlayPages !== 0) fail("machine-assisted candidates must not be accepted for P0 overlay");
if (batch.blockedUntilHumanReviewedPages !== 3) fail(`expected 3 human-review-blocked pages, got ${batch.blockedUntilHumanReviewedPages}`);
...
for (const flag of [
  ...
  "quiz_summary_language",
  "market_meaning_language",
