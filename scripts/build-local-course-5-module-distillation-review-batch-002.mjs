...
const outputJson = "docs/LOCAL_COURSE_5_MODULE_DISTILLATION_REVIEW_BATCH_002.json";
const outputMd = "docs/LOCAL_COURSE_5_MODULE_DISTILLATION_REVIEW_BATCH_002.md";
const inputCopyJson = "docs/reviewer-inputs/LOCAL_COURSE_5_MODULE_DISTILLATION_REVIEW_BATCH_002_INPUT_COPY_TEMPLATE.json";
const inputCopyMd = "docs/reviewer-inputs/LOCAL_COURSE_5_MODULE_DISTILLATION_REVIEW_BATCH_002_INPUT_COPY_TEMPLATE.md";

const boundary = "Course 5 module distillation review batch 002 is private reviewer-facing education operations material. It selects the next bounded set of visual/OCR cards not already covered by batch 001 and connects them to lesson seeds, evidence anchors, and reviewer input fields. It does not generate reviewer conclusions, accept machine drafts as human review, delete files, approve learner-facing release, copy private source wording, provide stock recommendations, live signals, return promises, broker workflows, automation, real-money guidance, write authorization, or production readiness.";
...
if (previousIds.size !== 40) fail("batch 001 coverage must be present before building batch 002");

const modules = new Map((distillation.moduleRows || []).map((row) => [row.moduleId, row]));
const rows = reviewerTemplate.rows
  .filter((row) => !previousIds.has(row.inputId))
...
const batchRows = selected.map((row, index) => {
  ...
  return {
    batchRowId: `course5_module_distillation_batch_002_row_${String(index + 1).padStart(3, "0")}`,
    batchId: "course5-module-distillation-review-batch-002",
...
const artifact = {
  ...
  approvalStatus: "not_approved",
  writeAllowedNow: false,
  batchStatus: "course_5_module_distillation_review_batch_002_ready_blocked_missing_real_input",
  batchId: "course5-module-distillation-review-batch-002",
  batchPurpose: "second_bounded_visual_ocr_review_pack_for_teaching_module_distillation",
  sourceDistillation: distillationPath,
  sourceReviewerTemplate: reviewerTemplatePath,
  previousBatch: previousBatchPath,
  totalTemplateRows: reviewerTemplate.inputRows,
  previouslySelectedRows: previousIds.size,
  remainingRowsBeforeBatch: reviewerTemplate.inputRows - previousIds.size,
  selectedRows: batchRows.length,
  remainingRowsAfterBatch: reviewerTemplate.inputRows - previousIds.size - batchRows.length,
  moduleRows,
  ...
  commands: [
    "npm.cmd run build:local-course-5-module-distillation-review-batch-002",
    "npm.cmd run check:local-course-5-module-distillation-review-batch-002",
    "npm.cmd run verify",
  ],
  completionRule: "Batch 002 is ready when it selects 40 Course 5 follow-up visual/OCR reviewer rows not already used by batch 001, maps each row to lesson seeds and evidence anchors, and keeps all release/delete fields blocked until real reviewer input is supplied and validated.",
  boundary,
};
...
const inputCopy = {
  ...
  approvalStatus: "not_approved",
  writeAllowedNow: false,
  inputCopyStatus: "course_5_module_distillation_review_batch_002_input_copy_ready_blank",
...
fs.writeFileSync(outputMd, [
  "# Local Course 5 Module Distillation Review Batch 002",
  "",
  `- Batch status: ${artifact.batchStatus}`,
  `- Selected rows: ${artifact.selectedRows}/${artifact.totalTemplateRows}`,
  `- Previously selected rows: ${artifact.previouslySelectedRows}`,
  `- Remaining rows after batch: ${artifact.remainingRowsAfterBatch}`,
...
fs.writeFileSync(inputCopyMd, [
  "# Local Course 5 Module Distillation Review Batch 002 Input Copy Template",
  "",
...
console.log(JSON.stringify({
  ...
  previouslySelectedRows: artifact.previouslySelectedRows,
  remainingRowsAfterBatch: artifact.remainingRowsAfterBatch,
