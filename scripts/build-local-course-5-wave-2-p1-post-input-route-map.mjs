...
const routeRows = validation.validationRows.map((row) => {
  ...
  const ready = row.readyForWave2SemanticReviewGate === true;
  ...
  return {
    ...
    readyForWave2SemanticReviewGate: ready,
    downstreamRouteMap,
    routeStatus: ready
      ? "ready_for_source_type_semantic_merge_preview_public_grounding_required"
      : "blocked_before_wave_2_semantic_merge_preview",
...
const readyRows = routeRows.filter((row) => row.readyForWave2SemanticReviewGate).length;
...
const routeMap = {
  ...
  routeMapStatus: readyRows > 0
    ? "course_5_wave_2_p1_post_input_route_map_has_ready_rows_release_and_deletion_blocked"
    : "course_5_wave_2_p1_post_input_route_map_ready_all_rows_blocked",
  routeMode: "post_input_route_map_for_wave_2_p1_reviewer_rows",
  waveId: pack.waveId,
  ...
  nextOperationalGates: [
    "Fill real Wave 2 reviewer-owned ZIP/PDF fields and re-run Wave 2 input validation.",
  ...
  commands: [
    "npm.cmd run validate:local-course-5-wave-2-p1-reviewer-input",
    "npm.cmd run check:local-course-5-wave-2-p1-reviewer-input-validation",
    "npm.cmd run build:local-course-5-wave-2-p1-post-input-route-map",
    "npm.cmd run check:local-course-5-wave-2-p1-post-input-route-map",
    "npm.cmd run verify",
  ],
  completionRule: "This route map is complete when all 15 Wave 2 validation rows are assigned source-type downstream routes, ready rows are isolated for future semantic merge preview, and all module/deletion gates remain closed until real accepted reviewer evidence and public grounding checks pass.",
  boundary,
};
...
<head>
  ...
  <title>Course 5 Wave 2 P1 Post-Input Route Map</title>
  <style>
...
<body>
  <header>
    <h1>Course 5 Wave 2 P1 Post-Input Route Map</h1>
