...
const outputJson = "docs/LESSON_BATCH_08_REVIEW_NOTES_DRY_RUN.json";
const outputMd = "docs/LESSON_BATCH_08_REVIEW_NOTES_DRY_RUN.md";
const paths = {
  editorPacket: "docs/LESSON_BATCH_08_EDITOR_PACKET.json",
};
...
function blankNoteCard(lesson) {
  return {
    ...
    notePrompts: {
      ...
      sourceFitNotes: "Record source-family role, downgrade, or block decisions; fraud and macro refs cannot prove chart/news outcomes.",
      factCheckNotes: "Record facts checked against source identity, release context, historical terminology, and event-reading limits.",
...
function validateDryRunOverlay(overlay) {
  assertEnvelope(overlay, "batch 08 dry-run overlay");
  if (overlay.dryRunOnly !== true) fail("dry-run overlay must keep dryRunOnly true");
  if (overlay.batchId !== TARGET_BATCH_ID) fail("dry-run overlay must stay scoped to rewrite_batch_08");
...
function markdown(report) {
  return [
    "# Lesson Batch 08 Review Notes Dry Run",
    "",
    "This is a blank reviewer-notes dry run for Batch 08.",
    "It prepares human notes for medium-heavy chart/news source-fit risk without creating real review evidence or changing lesson status.",
    "",
    "## Summary",
    "",
...
assertEnvelope(editorPacket, "batch 08 editor packet");
if (editorPacket.batchId !== TARGET_BATCH_ID) fail("editor packet must target rewrite_batch_08");
...
if ((editorPacket.riskCounts?.medium || 0) < 5) fail("editor packet must preserve batch 08 medium-risk focus");
...
const negativeRows = [
  negativeCase("filled_note_rejected", (overlay) => {
    overlay.lessonCards[0].sourceFitNotes = "Fraud source does not prove chart reading.";
  }, /must remain blank/),
  ...
  negativeCase("wrong_batch_rejected", (overlay) => {
    overlay.batchId = "rewrite_batch_07";
  }, /rewrite_batch_08/),
];
const failedNegativeRows = negativeRows.filter((row) => !row.passed);
if (failedNegativeRows.length) fail(`batch 08 notes dry-run negative cases failed: ${failedNegativeRows.map((row) => row.name).join(", ")}`);
...
const report = {
  ...
  negativeRows,
  sourceReports: paths,
  boundary: "This Batch 08 notes dry-run is a blank reviewer-input scaffold only. It does not create real review evidence, approve lessons, publish learner-facing content, change grades, certify commercial readiness, provide trading advice, imply performance, connect brokers, automate trading, guide real-money decisions, or make the product production-ready.",
};
