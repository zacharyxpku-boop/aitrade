...
const inputPath = argValue("--input", "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_4_PDF_REVIEWER_INPUT_TEMPLATE.json");
const packPath = argValue("--pack", "docs/LOCAL_COURSE_5_WAVE_4_PDF_EXECUTION_PACK.json");
const outputJsonPath = argValue("--output-json", "docs/LOCAL_COURSE_5_WAVE_4_PDF_REVIEWER_INPUT_VALIDATION.json");
const outputMdPath = argValue("--output-md", "docs/LOCAL_COURSE_5_WAVE_4_PDF_REVIEWER_INPUT_VALIDATION.md");
...
assertBoundary("Wave 4 PDF reviewer input", input);
assertBoundary("Wave 4 PDF execution pack", pack);

if (!Array.isArray(input.rows) || input.rows.length !== 27) fail("expected 27 Wave 4 PDF reviewer input rows");
if (!Array.isArray(pack.sampleRowsDetail) || pack.sampleRowsDetail.length !== 27) fail("expected 27 Wave 4 PDF execution samples");
...
const validationRows = input.rows.map((row) => {
  assertBoundary(`Wave 4 PDF reviewer input ${row.reviewRowId}`, row);
  ...
  return {
    ...
    validationStatus: rowIssues.length ? "blocked_missing_or_invalid_wave_4_pdf_reviewer_input" : "ready_for_wave_4_pdf_semantic_review_gate",
    readyForWave4PdfSemanticReviewGate: rowIssues.length === 0,
    missingFields,
    qualityIssues: rowIssues.filter((issue) => !issue.startsWith("missing_fields:")),
    forbiddenHits: hits,
    nextGate: rowIssues.length
      ? "fill_real_wave_4_pdf_reviewer_fields_then_revalidate"
      : "route_ready_wave_4_pdf_row_to_pdf_ocr_priority_route_map_after_public_grounding_check",
  };
});

const readyRows = validationRows.filter((row) => row.readyForWave4PdfSemanticReviewGate).length;
...
const report = {
  ...
  validationStatus: readyRows === validationRows.length
    ? "course_5_wave_4_pdf_reviewer_input_valid_release_still_locked"
    : "course_5_wave_4_pdf_reviewer_input_blocked_missing_or_invalid_real_input",
  validationMode: "wave_4_pdf_real_reviewer_input_gate",
  inputPath,
  ...
  forbiddenPhrases,
  validationRows,
  completionRule: "Wave 4 PDF reviewer input validation passes only when all 27 rows contain real reviewer-owned OCR and visual-semantic fields, no copied machine summaries, no forbidden trading advice language, public grounding remains required, and no release/delete boundary drifts.",
  boundary: "Course 5 Wave 4 PDF reviewer input validation is private reviewer-facing education operations material. It validates reviewer-owned input for the nine course-core PDF OCR blockers and their representative page samples. It does not perform OCR, generate reviewer conclusions, accept machine drafts as human review, delete files, approve learner-facing release, approve source-folder deletion, copy private source wording, provide stock recommendations, live signals, return promises, broker workflows, automation, real-money guidance, write authorization, or production readiness.",
};
...
fs.writeFileSync(outputMdPath, [
  ...
  "",
  ...validationRows
    .filter((row) => !row.readyForWave4PdfSemanticReviewGate)
    .slice(0, 27)
