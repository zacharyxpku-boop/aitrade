...
const inputPath = "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_2_P1_AI_VISUAL_REVIEW_PILOT_INPUT.json";
const inputMdPath = "docs/reviewer-inputs/LOCAL_COURSE_5_WAVE_2_P1_AI_VISUAL_REVIEW_PILOT_INPUT.md";
const validationPath = "docs/LOCAL_COURSE_5_WAVE_2_P1_AI_VISUAL_REVIEW_PILOT_VALIDATION.json";
...
if (input.inputTemplateStatus !== "course_5_wave_2_p1_ai_visual_review_pilot_twelve_rows_ready_for_reviewer_confirmation") fail("unexpected pilot input status");
if (validation.validationStatus !== "course_5_wave_2_p1_ai_visual_review_pilot_partially_ready_release_and_deletion_blocked") fail("unexpected validation status");
if (validation.validationMode !== "wave_2_p1_ai_visual_review_pilot_gate") fail("unexpected validation mode");
if (input.rows.length !== 15 || validation.inputRows !== 15) fail("pilot must keep all 15 Wave 2 rows");
if (input.pilotRows !== 12 || validation.pilotRows !== 12) fail("pilot must cover exactly 12 rows");
if (validation.readyRows !== 12 || validation.blockedRows !== 3) fail("pilot ready/blocked counts drift");
if (validation.missingFieldRows !== 3 || validation.forbiddenHitRows !== 0) fail("pilot validation quality counts drift");
if (validation.acceptedForWave2SemanticReviewRows !== 0 || validation.acceptedForModuleDistillationRows !== 0 || validation.acceptedForDeletionReadinessRows !== 0) fail("pilot must not accept rows for merge/deletion");
if (validation.learnerReadyModules !== 0) fail("pilot must not create learner-ready modules");

const readyRows = validation.validationRows.filter((row) => row.readyForWave2AiVisualConfirmationGate);
...
for (const row of input.rows.filter((row) => readyIds.includes(row.reviewRowId))) {
  ...
  if (row.acceptedForWave2SemanticReview !== false || row.acceptedForModuleDistillation !== false || row.acceptedForDeletionReadiness !== false) fail(`pilot row must not self-accept: ${row.reviewRowId}`);
}
