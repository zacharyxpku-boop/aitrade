...
const lintPath = "docs/LOCAL_COURSE_P0_HUMAN_FILL_PACK_01_INPUT_COPY_LINT.json";
...
if (lint.packId !== "local_course_p0_human_fill_pack_01") fail(`unexpected packId: ${lint.packId}`);
...
for (const row of rows) {
  ...
  if (!Array.isArray(row.riskRewriteMissingFlags) || row.riskRewriteMissingFlags.length < 4) fail(`${row.id} should report missing risk rewrites`);
