...
const routeMapPath = "docs/LOCAL_COURSE_5_WAVE_4_PDF_POST_INPUT_ROUTE_MAP.json";
const routeMapMdPath = "docs/LOCAL_COURSE_5_WAVE_4_PDF_POST_INPUT_ROUTE_MAP.md";
const routeMapHtmlPath = "docs/local-course-5-wave-4-pdf-post-input-route-map.html";
...
if (routeMap.routeMapStatus !== "course_5_wave_4_pdf_post_input_route_map_ready_all_rows_blocked") fail(`unexpected routeMapStatus: ${routeMap.routeMapStatus}`);
if (routeMap.routeMode !== "post_input_route_map_for_wave_4_pdf_reviewer_rows") fail("unexpected route mode");
if (routeMap.waveId !== "wave_4_course_core_pdf_ocr_blockers") fail("unexpected wave id");
if (routeMap.routeRows !== 27 || routeMap.pdfRouteRows !== 27 || routeMap.zipRouteRows !== 0) fail("route counts drift");
if (routeMap.readyRows !== 0 || routeMap.blockedRows !== 27 || routeMap.semanticMergePreviewReadyRows !== 0) fail("blank Wave 4 PDF route rows should be blocked");
...
if (routeMap.acceptedForWave4PdfSemanticReviewRows !== 0 || routeMap.acceptedForModuleDistillationRows !== 0 || routeMap.acceptedForDeletionReadinessRows !== 0) fail("route map must not accept rows");
...
if (!Array.isArray(routeMap.routeRowsDetail) || routeMap.routeRowsDetail.length !== 27) fail("routeRowsDetail must cover 27 rows");
...
for (const row of routeMap.routeRowsDetail) {
  ...
  if (row.readyForWave4PdfSemanticReviewGate !== false) fail(`route row should not be ready: ${row.reviewRowId}`);
  if (row.routeStatus !== "blocked_before_wave_4_pdf_semantic_merge_preview") fail(`route row should be blocked: ${row.reviewRowId}`);
...
for (const phrase of [
  ...
  "validation-ready wave 4 pdf reviewer rows",
