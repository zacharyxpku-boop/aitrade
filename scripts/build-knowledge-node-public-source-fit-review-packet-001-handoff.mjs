...
const outputJson = "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_001_HANDOFF.json";
const outputMd = "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_001_HANDOFF.md";

const sources = {
  inputTemplate: "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_INPUT_COPY_TEMPLATE.json",
  packetValidation: "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_INPUT_COPY_TEMPLATE_VALIDATION.json",
  mergePreview: "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_MERGE_PREVIEW.json",
  mergeApplyReport: "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_MERGE_APPLY_REPORT.json",
...
const phaseRows = [
  {
    order: 1,
    id: "inspect_packet_input_copy_template",
    status: data.inputTemplate.templateStatus,
    inputFile: sources.inputTemplate,
    command: "npm.cmd run check:knowledge-node-public-source-fit-review-packet-input-copy-template",
    reviewerAction: "Open the packet input copy template and fill only reviewerDecision, sourceFitNotes, citationUse, reviewerName, reviewedAt, and realHumanInput for rows you personally reviewed.",
    hardStop: "Stop if the file is not packet 001, has fewer than 60 rows, or any row already claims learnerCitationApproved/copy approval.",
  },
  {
    order: 2,
    id: "validate_packet_input_copy",
    status: data.packetValidation.validationStatus,
    inputFile: sources.packetValidation,
    command: "npm.cmd run validate:knowledge-node-public-source-fit-review-packet-input-copy-template && npm.cmd run check:knowledge-node-public-source-fit-review-packet-input-copy-template",
    reviewerAction: "Rerun packet validation after filling; all 60 rows must be ready before merge preview can proceed.",
    hardStop: "Stop while readyRows is 0, blockedRows is 60, missingFieldRows is 60, or realHumanInputEntries is 0.",
  },
  {
    order: 3,
    id: "rebuild_merge_preview",
    status: data.mergePreview.mergePreviewStatus,
    inputFile: sources.mergePreview,
    command: "npm.cmd run build:knowledge-node-public-source-fit-review-packet-merge-preview && npm.cmd run check:knowledge-node-public-source-fit-review-packet-merge-preview",
    reviewerAction: "Confirm every packet row maps back to the 1638-row full source-fit draft before any merge apply.",
    hardStop: "Stop unless mappedRows is 60, missingTargetRows is 0, and mergeAllowedNow is true after real review.",
  },
  {
    order: 4,
    id: "run_merge_apply_dry_run",
    status: data.mergeApplyReport.applyStatus,
    inputFile: sources.mergeApplyReport,
    command: "npm.cmd run apply:knowledge-node-public-source-fit-review-packet-merge && npm.cmd run check:knowledge-node-public-source-fit-review-packet-merge-apply-report",
    reviewerAction: "Use dry-run apply to verify willWrite stays false until the packet is fully ready and explicit write authorization exists.",
  ...
  },
  {
    ...
    reviewerAction: "After an authorized merge, rerun the packet/module progress matrix and confirm packet 001 moves from blocked to ready.",
...
const handoff = {
  ...
  handoffStatus: "node_public_source_fit_packet_001_handoff_ready_blocked_on_real_input",
  ...
  commands: [
    "npm.cmd run check:knowledge-node-public-source-fit-review-packet-001-handoff",
