...
const outputJson = "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_MERGE_PREVIEW.json";
const outputMd = "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_MERGE_PREVIEW.md";
...
const template = readJson("docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_INPUT_COPY_TEMPLATE.json");
const packetInput = readJson("docs/reviewer-inputs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_001_INPUT_COPY_TEMPLATE.json");
const packetValidation = readJson("docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_INPUT_COPY_TEMPLATE_VALIDATION.json");
...
const mergeRows = (packetInput.rows || []).map((row, packetRowIndex) => {
  ...
  return {
    ...
    fieldCopyPlan: {
      reviewerDecision: {
        from: `/rows/${packetRowIndex}/reviewerDecision`,
        to: `/rows/${fullMatch?.index ?? "missing"}/reviewerDecision`,
      },
      sourceFitNotes: {
        from: `/rows/${packetRowIndex}/sourceFitNotes`,
        to: `/rows/${fullMatch?.index ?? "missing"}/sourceFitNotes`,
      },
      citationUse: {
        from: `/rows/${packetRowIndex}/citationUse`,
        to: `/rows/${fullMatch?.index ?? "missing"}/citationUse`,
      },
      reviewerName: {
        from: `/rows/${packetRowIndex}/reviewerName`,
        to: `/rows/${fullMatch?.index ?? "missing"}/reviewerName`,
      },
      reviewedAt: {
        from: `/rows/${packetRowIndex}/reviewedAt`,
        to: `/rows/${fullMatch?.index ?? "missing"}/reviewedAt`,
      },
      realHumanInput: {
        from: `/rows/${packetRowIndex}/realHumanInput`,
        to: `/rows/${fullMatch?.index ?? "missing"}/realHumanInput`,
      },
    },
  };
});
...
const preview = {
  ...
  mergePreviewStatus: mergeAllowedNow
    ? "packet_merge_preview_ready_for_manual_authorization"
    : "packet_merge_preview_blocked_missing_ready_packet_input",
  ...
  packetInputPath: "docs/reviewer-inputs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_001_INPUT_COPY_TEMPLATE.json",
  packetValidationPath: "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_INPUT_COPY_TEMPLATE_VALIDATION.json",
