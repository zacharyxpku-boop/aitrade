...
for (const [name, artifact] of Object.entries({ validation, pack, zipRoute, deletion })) {
...
const routeRows = validation.validationRows.map((row) => {
  ...
  const ready = row.readyForWave3SemanticReviewGate === true;
  return {
    ...
    readyForWave3SemanticReviewGate: ready,
    downstreamRouteMap: zipRoutePath,
    routeStatus: ready
      ? "ready_for_zip_semantic_merge_preview_public_grounding_required"
      : "blocked_before_wave_3_zip_semantic_merge_preview",
    semanticMergePreviewAllowedNow: ready,
    moduleDistillationAllowedNow: false,
    deletionEvidenceAllowedNow: false,
    nextGate: ready
      ? "route_to_zip_priority_route_map_then_public_grounding_review"
      : row.nextGate,
...
const readyRows = routeRows.filter((row) => row.readyForWave3SemanticReviewGate).length;
...
const routeMap = {
  ...
  routeMapStatus: readyRows > 0
    ? "course_5_wave_3_zip_post_input_route_map_has_ready_rows_release_and_deletion_blocked"
    : "course_5_wave_3_zip_post_input_route_map_ready_all_rows_blocked",
  routeMode: "post_input_route_map_for_wave_3_zip_reviewer_rows",
  waveId: pack.waveId,
  ...
  nextOperationalGates: [
    "Fill real Wave 3 reviewer-owned ZIP visual fields and re-run Wave 3 input validation.",
  ...
  commands: [
    "npm.cmd run validate:local-course-5-wave-3-zip-reviewer-input",
    "npm.cmd run check:local-course-5-wave-3-zip-reviewer-input-validation",
    "npm.cmd run build:local-course-5-wave-3-zip-post-input-route-map",
    "npm.cmd run check:local-course-5-wave-3-zip-post-input-route-map",
    "npm.cmd run verify",
  ],
  completionRule: "This route map is complete when all 61 Wave 3 ZIP validation rows are assigned downstream ZIP routes, ready rows are isolated for future semantic merge preview, and all module/deletion gates remain closed until real accepted reviewer evidence and public grounding checks pass.",
  boundary,
};

fs.writeFileSync(outputJson, `${JSON.stringify(routeMap, null, 2)}\n`, "utf8");
fs.writeFileSync(outputMd, [
  "# Course 5 Wave 3 ZIP Post-Input Route Map",
  "",
  `- Route map status: ${routeMap.routeMapStatus}`,
  `- Route rows: ${routeMap.routeRows}`,
  `- ZIP route rows: ${routeMap.zipRouteRows}`,
  ...
  "## First Rows",
  "",
  "| Sample | Status | Downstream route | Next gate |",
  "|---:|---|---|---|",
  ...routeRows.slice(0, 30).map((row) => `| ${row.executionSampleNo} | ${row.routeStatus} | ${row.downstreamRouteMap} | ${row.nextGate} |`),
  "",
  "## Boundary",
  "",
  boundary,
...
<head>
  ...
  <title>Course 5 Wave 3 ZIP Post-Input Route Map</title>
  <style>
...
<body>
  <header>
    <h1>Course 5 Wave 3 ZIP Post-Input Route Map</h1>
  ...
  <main>
    <table>
      <thead><tr><th>Sample</th><th>Status</th><th>Downstream Route</th><th>Next Gate</th></tr></thead>
      <tbody>${rowHtml}</tbody>
    </table>
  </main>
