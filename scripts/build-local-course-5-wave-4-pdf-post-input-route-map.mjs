...
const validationPath = "docs/LOCAL_COURSE_5_WAVE_4_PDF_REVIEWER_INPUT_VALIDATION.json";
...
const routeRows = validation.validationRows.map((row) => {
  ...
  const ready = row.readyForWave4PdfSemanticReviewGate === true;
  return {
    ...
    readyForWave4PdfSemanticReviewGate: ready,
    downstreamRouteMap: pdfRoutePath,
    routeStatus: ready
      ? "ready_for_pdf_semantic_merge_preview_public_grounding_required"
      : "blocked_before_wave_4_pdf_semantic_merge_preview",
    semanticMergePreviewAllowedNow: ready,
    moduleDistillationAllowedNow: false,
    deletionEvidenceAllowedNow: false,
    nextGate: ready
      ? "route_to_pdf_ocr_priority_route_map_then_public_grounding_review"
      : row.nextGate,
...
const readyRows = routeRows.filter((row) => row.readyForWave4PdfSemanticReviewGate).length;
...
const routeMap = {
  ...
  routeMapStatus: readyRows > 0
    ? "course_5_wave_4_pdf_post_input_route_map_has_ready_rows_release_and_deletion_blocked"
    : "course_5_wave_4_pdf_post_input_route_map_ready_all_rows_blocked",
  routeMode: "post_input_route_map_for_wave_4_pdf_reviewer_rows",
  waveId: pack.waveId,
  ...
  nextOperationalGates: [
    "Fill real Wave 4 PDF reviewer-owned OCR and visual-semantic fields and re-run Wave 4 input validation.",
  ...
  commands: [
    "npm.cmd run validate:local-course-5-wave-4-pdf-reviewer-input",
    "npm.cmd run check:local-course-5-wave-4-pdf-reviewer-input-validation",
    "npm.cmd run build:local-course-5-wave-4-pdf-post-input-route-map",
    "npm.cmd run check:local-course-5-wave-4-pdf-post-input-route-map",
    "npm.cmd run verify",
  ],
  completionRule: "This route map is complete when all 27 Wave 4 PDF validation rows are assigned the PDF downstream route, ready rows are isolated for future semantic merge preview, and all module/deletion gates remain closed until real accepted reviewer evidence and public grounding checks pass.",
  boundary,
};

fs.writeFileSync(outputJson, `${JSON.stringify(routeMap, null, 2)}\n`, "utf8");
fs.writeFileSync(outputMd, [
  "# Course 5 Wave 4 PDF Post-Input Route Map",
  "",
...
<head>
  ...
  <title>Course 5 Wave 4 PDF Post-Input Route Map</title>
  <style>
...
<body>
  <header>
    <h1>Course 5 Wave 4 PDF Post-Input Route Map</h1>
