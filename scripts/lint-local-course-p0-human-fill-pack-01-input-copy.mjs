...
const defaultInputPath = "docs/LOCAL_COURSE_P0_HUMAN_FILL_PACK_01_INPUT_COPY_TEMPLATE.json";
const defaultOutputJsonPath = "docs/LOCAL_COURSE_P0_HUMAN_FILL_PACK_01_INPUT_COPY_LINT.json";
const defaultOutputMdPath = "docs/LOCAL_COURSE_P0_HUMAN_FILL_PACK_01_INPUT_COPY_LINT.md";
...
if (input.packId !== "local_course_p0_human_fill_pack_01") fail(`unexpected packId: ${input.packId}`);
...
const lintRows = (input.inputEntries || []).map((entry) => {
  ...
  const publicReferenceMissing = !/public|wikipedia|official|reference|source|公开|维基|来源|参考/i.test(text(entry.manualInput?.publicReferenceNotes));
  const originalityMissing = !/not copied|original|paraphrase|rewrite|原创|改写|不复制/i.test(text(entry.manualInput?.originalityNotes));
...
const report = {
  ...
  boundary: "Pack 01 filled-copy lint is a dry-run quality gate. It does not write overlay changes, perform OCR, approve learner-facing release, copy private course wording into lessons, provide trading advice, stock recommendations, live signals, return promises, broker workflows, automation, or real-money guidance.",
};

fs.writeFileSync(outputJsonPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
fs.writeFileSync(outputMdPath, [
  "# Local Course P0 Human Fill Pack 01 Input Copy Lint",
  "",
  "Dry-run quality lint for filled pack 01 reviewer input copies.",
  "",
