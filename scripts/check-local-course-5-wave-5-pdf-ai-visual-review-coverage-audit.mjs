...
const auditPath = "docs/LOCAL_COURSE_5_WAVE_5_PDF_AI_VISUAL_REVIEW_COVERAGE_AUDIT.json";
const auditMdPath = "docs/LOCAL_COURSE_5_WAVE_5_PDF_AI_VISUAL_REVIEW_COVERAGE_AUDIT.md";
...
if (audit.auditStatus !== "course_5_wave_5_pdf_ai_visual_review_all_85_samples_covered_release_and_deletion_blocked") fail("unexpected audit status");
if (audit.batchCount !== 7) fail(`expected 7 batches, got ${audit.batchCount}`);
if (audit.templateRows !== 85) fail(`expected 85 template rows, got ${audit.templateRows}`);
if (audit.coveredRows !== 85) fail(`expected 85 covered rows, got ${audit.coveredRows}`);
...
if (audit.readyForHumanConfirmationRows !== 85) fail(`expected 85 ready rows, got ${audit.readyForHumanConfirmationRows}`);
...
if (audit.sourceFolderMayBeDeleted !== false) fail("source folder removal must remain blocked");
if (!Array.isArray(audit.coverageRows) || audit.coverageRows.length !== 85) fail("coverage rows must contain 85 rows");
