...
const templatePath = "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_INPUT_COPY_TEMPLATE.json";
const inputCopyPath = "docs/reviewer-inputs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_001_INPUT_COPY_TEMPLATE.json";
const validationPath = "docs/KNOWLEDGE_NODE_PUBLIC_SOURCE_FIT_REVIEW_PACKET_INPUT_COPY_TEMPLATE_VALIDATION.json";
...
if (template.templateStatus !== "node_public_source_fit_packet_input_copy_template_ready_blank") {
  fail(`unexpected template status: ${template.templateStatus}`);
}
if (template.templateMode !== "first_blocked_packet_scoped_input_copy_for_real_reviewer") fail("unexpected template mode");
...
if (template.packetId !== "node-public-source-fit-batch-001-packet") fail("template should target the first blocked packet");
...
if (!Array.isArray(template.commands) || !template.commands.some((item) => item.includes("validate:knowledge-node-public-source-fit-review-packet-input-copy-template"))) {
  fail("template commands missing packet validation command");
}
