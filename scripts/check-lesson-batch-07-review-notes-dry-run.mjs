...
const outputJson = "docs/LESSON_BATCH_07_REVIEW_NOTES_DRY_RUN.json";
const outputMd = "docs/LESSON_BATCH_07_REVIEW_NOTES_DRY_RUN.md";
const paths = {
  editorPacket: "docs/LESSON_BATCH_07_EDITOR_PACKET.json",
};
...
function blankNoteCard(lesson) {
  return {
    ...
    notePrompts: {
      ...
      sourceFitNotes: "Record each source-family role, downgrade, or block decision; macro, filing, and public-domain context cannot become chart-pattern proof.",
      factCheckNotes: "Record facts checked against source identity, release context, historical terminology, and lesson claims.",
      ...
      copyingRiskNotes: "Confirm no external body text, tables, charts, UI text, public-domain market maxims, or sample notes were copied.",
...
function validateCard(card, { allowFilledNotes = false } = {}) {
  ...
  for (const field of REQUIRED_NOTE_FIELDS) {
    ...
    if (!allowFilledNotes && value !== "") fail(`${card.lessonId}.${field} must remain blank in dry-run`);
  }
}
...
function validateDryRunOverlay(overlay) {
  assertEnvelope(overlay, "batch 07 dry-run overlay");
  if (overlay.dryRunOnly !== true) fail("dry-run overlay must keep dryRunOnly true");
  if (overlay.batchId !== TARGET_BATCH_ID) fail("dry-run overlay must stay scoped to rewrite_batch_07");
...
function markdown(report) {
  return [
    "# Lesson Batch 07 Review Notes Dry Run",
    "",
    "This is a blank reviewer-notes dry run for Batch 07.",
    "It prepares human source-fit, fact-check, boundary, and copying-risk notes without creating real review evidence or changing lesson status.",
    "",
    "## Summary",
    "",
    ...
    `- Negative cases passed: ${report.negativeCasesPassed}`,
    ...
    "## Blank Note Rows",
    "",
    "| Lesson | Module | Topic | Risk | Blank fields | Status |",
    "| --- | --- | --- | --- | ---: | --- |",
    ...report.blankRows.map((row) => `| ${row.lessonId} | ${row.moduleLabel} | ${row.topicLabel} | ${row.sourceFitRisk} | ${row.blankFields} | ${row.trackingStatus} |`),
    "",
    "## Negative Cases",
    "",
...
assertEnvelope(editorPacket, "batch 07 editor packet");
if (editorPacket.batchId !== TARGET_BATCH_ID) fail("editor packet must target rewrite_batch_07");
...
const negativeRows = [
  negativeCase("filled_note_rejected", (overlay) => {
    overlay.lessonCards[0].sourceFitNotes = "Source family role checked.";
  }, /must remain blank/),
  negativeCase("approval_candidate_rejected", (overlay) => {
    overlay.approvalReviewCandidates = 1;
  }, /approval review candidates/),
  negativeCase("commercial_promotion_rejected", (overlay) => {
    overlay.commercialReadyPromotions = 1;
  }, /commercial readiness/),
  negativeCase("learner_release_rejected", (overlay) => {
    overlay.learnerFacingRelease = true;
  }, /learner-facing release/),
  negativeCase("grade_override_rejected", (overlay) => {
    overlay.lessonCards[0].currentGrade = "commercial_ready";
  }, /currentGrade/),
  negativeCase("signal_wording_rejected", (overlay) => {
    overlay.lessonCards[0].boundaryCheckNotes = "This confirms a breakout signal.";
  }, /unsafe status\/trading\/readiness wording/),
  negativeCase("duplicate_lesson_rejected", (overlay) => {
    overlay.lessonCards.push({ ...overlay.lessonCards[0] });
  }, /6 lesson cards|duplicate/),
  negativeCase("wrong_batch_rejected", (overlay) => {
    overlay.batchId = "rewrite_batch_06";
  }, /rewrite_batch_07/),
];
const failedNegativeRows = negativeRows.filter((row) => !row.passed);
if (failedNegativeRows.length) fail(`batch 07 notes dry-run negative cases failed: ${failedNegativeRows.map((row) => row.name).join(", ")}`);
...
const filledNoteFields = dryRunOverlay.lessonCards.reduce((sum, card) => sum + REQUIRED_NOTE_FIELDS.filter((field) => hasText(card[field])).length, 0);
...
const report = {
  ...
  blankNoteFields,
  filledNoteFields,
  approvalReviewCandidates: dryRunOverlay.approvalReviewCandidates,
  commercialReadyPromotions: dryRunOverlay.commercialReadyPromotions,
  blankRows,
  dryRunOverlay,
  ...
  negativeRows,
  sourceReports: paths,
  boundary: "This Batch 07 notes dry-run is a blank reviewer-input scaffold only. It does not create real review evidence, approve lessons, publish learner-facing content, change grades, certify commercial readiness, provide trading advice, imply performance, connect brokers, automate trading, guide real-money decisions, or make the product production-ready.",
};
