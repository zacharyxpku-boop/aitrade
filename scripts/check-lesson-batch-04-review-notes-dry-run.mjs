...
const outputJson = "docs/LESSON_BATCH_04_REVIEW_NOTES_DRY_RUN.json";
const outputMd = "docs/LESSON_BATCH_04_REVIEW_NOTES_DRY_RUN.md";
const paths = { editorPacket: "docs/LESSON_BATCH_04_EDITOR_PACKET.json" };
...
function blankNoteCard(lesson) {
  return {
    ...
    notePrompts: {
      ...
      sourceFitNotes: "Record event/data/regulatory/fraud/public-domain source-fit decisions; chart rows need downgrade or boundary-only notes.",
...
function validateDryRunOverlay(overlay) {
  assertEnvelope(overlay, "batch 04 dry-run overlay");
  if (overlay.dryRunOnly !== true) fail("dry-run overlay must keep dryRunOnly true");
  if (overlay.batchId !== TARGET_BATCH_ID) fail("dry-run overlay must stay scoped to rewrite_batch_04");
...
function markdown(report) {
  return [
    "# Lesson Batch 04 Review Notes Dry Run",
    "",
    "This is a blank reviewer-notes dry run for Batch 04 market-structure/news/candlestick source-fit review.",
    "",
    "## Summary",
    "",
...
assertEnvelope(editorPacket, "batch 04 editor packet");
if (editorPacket.batchId !== TARGET_BATCH_ID) fail("editor packet must target rewrite_batch_04");
...
if ((editorPacket.riskCounts?.medium || 0) < 1) fail("editor packet must preserve medium-risk focus");
...
const negativeRows = [
  ...
  negativeCase("signal_wording_rejected", (overlay) => { overlay.lessonCards[0].boundaryCheckNotes = "This confirms a structure-break signal."; }, /unsafe status\/trading\/readiness wording/),
  negativeCase("duplicate_lesson_rejected", (overlay) => { overlay.lessonCards.push({ ...overlay.lessonCards[0] }); }, /6 lesson cards|duplicate/),
  negativeCase("wrong_batch_rejected", (overlay) => { overlay.batchId = "rewrite_batch_02"; }, /rewrite_batch_04/),
];
const failedNegativeRows = negativeRows.filter((row) => !row.passed);
if (failedNegativeRows.length) fail(`batch 04 notes dry-run negative cases failed: ${failedNegativeRows.map((row) => row.name).join(", ")}`);
...
const report = {
  ...
  negativeRows,
  sourceReports: paths,
  boundary: "This Batch 04 notes dry-run is a blank reviewer-input scaffold only. It does not create real review evidence, approve lessons, publish learner-facing content, change grades, certify commercial readiness, provide trading advice, imply performance, connect brokers, automate trading, guide real-money decisions, or make the product production-ready.",
};
