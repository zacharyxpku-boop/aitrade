...
const outputJson = "docs/LESSON_BATCH_06_REVIEW_NOTES_DRY_RUN.json";
const outputMd = "docs/LESSON_BATCH_06_REVIEW_NOTES_DRY_RUN.md";
const paths = {
  editorPacket: "docs/LESSON_BATCH_06_EDITOR_PACKET.json",
};
...
function blankNoteCard(lesson) {
  return {
    ...
    notePrompts: {
      ...
      sourceFitNotes: "Record each source-family role, downgrade, or block decision; green refs remain evidence boundaries, not learner-facing proof by themselves.",
      ...
      copyingRiskNotes: "Confirm no external body text, tables, boilerplate, UI text, old market maxims, or sample notes were copied.",
...
function validateDryRunOverlay(overlay) {
  assertEnvelope(overlay, "batch 06 dry-run overlay");
  if (overlay.dryRunOnly !== true) fail("dry-run overlay must keep dryRunOnly true");
  if (overlay.batchId !== TARGET_BATCH_ID) fail("dry-run overlay must stay scoped to rewrite_batch_06");
...
function markdown(report) {
  return [
    "# Lesson Batch 06 Review Notes Dry Run",
    "",
    "This is a blank reviewer-notes dry run for Batch 06.",
    "It helps a human editor start source-fit, fact-check, boundary, and copying-risk notes without creating real review evidence or changing lesson status.",
    "",
    "## Summary",
    "",
    ...
    "## Blank Note Rows",
    "",
    "| Lesson | Module | Topic | Blank fields | Status |",
    "| --- | --- | --- | ---: | --- |",
    ...report.blankRows.map((row) => `| ${row.lessonId} | ${row.moduleLabel} | ${row.topicLabel} | ${row.blankFields} | ${row.trackingStatus} |`),
    "",
    "## Negative Cases",
    "",
...
assertEnvelope(editorPacket, "batch 06 editor packet");
if (editorPacket.batchId !== TARGET_BATCH_ID) fail("editor packet must target rewrite_batch_06");
...
const negativeRows = [
  negativeCase("filled_note_rejected", (overlay) => {
    overlay.lessonCards[0].sourceFitNotes = "Looks fine.";
  }, /must remain blank/),
  ...
  negativeCase("signal_wording_rejected", (overlay) => {
    overlay.lessonCards[0].boundaryCheckNotes = "This creates a trading signal.";
  ...
  negativeCase("wrong_batch_rejected", (overlay) => {
    overlay.batchId = "rewrite_batch_01";
  }, /rewrite_batch_06/),
];
const failedNegativeRows = negativeRows.filter((row) => !row.passed);
if (failedNegativeRows.length) fail(`batch 06 notes dry-run negative cases failed: ${failedNegativeRows.map((row) => row.name).join(", ")}`);
...
const report = {
  ...
  negativeRows,
  sourceReports: paths,
  boundary: "This Batch 06 notes dry-run is a blank reviewer-input scaffold only. It does not create real review evidence, approve lessons, publish learner-facing content, change grades, certify commercial readiness, provide trading advice, imply performance, connect brokers, automate trading, guide real-money decisions, or make the product production-ready.",
};
