...
const outputJson = "docs/LESSON_BATCH_05_REVIEW_NOTES_DRY_RUN.json";
const outputMd = "docs/LESSON_BATCH_05_REVIEW_NOTES_DRY_RUN.md";
const paths = { editorPacket: "docs/LESSON_BATCH_05_EDITOR_PACKET.json" };
...
function blankNoteCard(lesson) {
  return {
    ...
    notePrompts: {
      ...
      sourceFitNotes: "For K-line and risk rows, record downgrade/block/boundary-only decisions; fraud/regulatory/macro refs cannot prove price-action claims.",
...
function validateDryRunOverlay(overlay) {
  assertEnvelope(overlay, "batch 05 dry-run overlay");
  if (overlay.dryRunOnly !== true) fail("dry-run overlay must keep dryRunOnly true");
  if (overlay.batchId !== TARGET_BATCH_ID) fail("dry-run overlay must stay scoped to rewrite_batch_05");
...
function markdown(report) {
  return [
    "# Lesson Batch 05 Review Notes Dry Run",
    "",
    "This is a blank reviewer-notes dry run for Batch 05 high-risk source-fit review.",
    "",
    "## Summary",
    "",
...
assertEnvelope(editorPacket, "batch 05 editor packet");
if (editorPacket.batchId !== TARGET_BATCH_ID) fail("editor packet must target rewrite_batch_05");
...
if ((editorPacket.riskCounts?.high || 0) < 1) fail("editor packet must preserve high-risk focus");

const dryRunOverlay = buildDryRunOverlay();
validateDryRunOverlay(dryRunOverlay);
const negativeRows = [
  negativeCase("filled_note_rejected", (overlay) => { overlay.lessonCards[0].sourceFitNotes = "Downgrade fraud source to boundary-only."; }, /must remain blank/),
  ...
  negativeCase("signal_wording_rejected", (overlay) => { overlay.lessonCards[0].boundaryCheckNotes = "This confirms a liquidity sweep signal."; }, /unsafe status\/trading\/readiness wording/),
  negativeCase("duplicate_lesson_rejected", (overlay) => { overlay.lessonCards.push({ ...overlay.lessonCards[0] }); }, /6 lesson cards|duplicate/),
  negativeCase("wrong_batch_rejected", (overlay) => { overlay.batchId = "rewrite_batch_01"; }, /rewrite_batch_05/),
];
const failedNegativeRows = negativeRows.filter((row) => !row.passed);
if (failedNegativeRows.length) fail(`batch 05 notes dry-run negative cases failed: ${failedNegativeRows.map((row) => row.name).join(", ")}`);
...
const report = {
  ...
  negativeRows,
  sourceReports: paths,
  boundary: "This Batch 05 notes dry-run is a blank reviewer-input scaffold only. It does not create real review evidence, approve lessons, publish learner-facing content, change grades, certify commercial readiness, provide trading advice, imply performance, connect brokers, automate trading, guide real-money decisions, or make the product production-ready.",
};
